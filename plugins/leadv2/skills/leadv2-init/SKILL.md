...
Write `codex-policy.yaml`:
```yaml
# .claude/leadv2-overrides/codex-policy.yaml — generated by leadv2-init
# Default: Codex disabled. Set codex_enabled: true to enable 2nd-brain reviews.
# Requires Codex CLI on PATH; see docs/INSTALLATION.md.
codex_enabled: false
codex_model: gpt-5.5
codex_review_rounds_max: 2
codex_planner_max_findings: 5
```

Write `state-paths.yaml`:
```yaml
...
- Prompt user to fill in: "DB details, hosting access, CI/CD URLs, deploy credentials"
