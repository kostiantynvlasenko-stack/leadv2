...
| `/leadv2 supervise` | **Supervisor mode (D-f=A, fat interactive supervisor — SUPERVISE-V2-01).** Lead: (1) reconciles+adopts existing work via `scripts/leadv2-supervise.sh --json` (triple-proof tmux adoption, corroborated-death tombstone+prune, F2 truth-probe; eligible adoptions/prunes suppressed by `observe_only` are still reported, never silently dropped) -> (2) `scripts/leadv2-supervise-pick.sh` ranks candidates, founder picks 0-5 via `AskUserQuestion` multiSelect -> (3) runs picked tasks as IN-SESSION `Workflow`/background-`Agent` lanes (own worktree each, registered in active.yaml with `backend: workflow` + `provider_receipts`) — this is the default execution architecture, not N autonomous `claude -p` leads -> (4) attaches `scripts/leadv2-supervise-loop.sh --ensure` via `Monitor` (5s event poll / 300s pulse, PID+birth sentinel, idempotent re-attach). Questions surface instantly via `AskUserQuestion`; all other output is relayed verbatim from loop lines, zero narration; a corroborated-dead lane is NEVER auto-restarted (escalated via `scripts/leadv2-ask.sh` with inspect/restart/abandon). Answers route via `scripts/leadv2-answer.sh` (canonical, cross-worktree) or `/leadv2 reply <q-id> <option>` for same-session legacy questions. See `.claude/skills/leadv2-supervise/SKILL.md`. |
| `/leadv2 fanout [--n N] [--backend tmux\|headless]` | **Compatibility-only** dispatch: launch N child sessions via `scripts/leadv2-fanout.sh` and exit — no picker, no reconciliation, no supervision loop attached. Not the default `/leadv2 supervise` path since D-f=A; existing/adopted tmux+headless children remain observed by the same `leadv2-supervise.sh`/`leadv2-supervise-loop.sh`. Concurrency ceiling is `active.yaml meta.hard_limit` (configurable; `--force` to exceed with a warning). Merges into main are serialized via `scripts/leadv2-merge-queue.sh` — a child MUST acquire the merge lock in Phase 6. |
