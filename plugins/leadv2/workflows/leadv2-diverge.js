export const meta = {
  name: 'leadv2-diverge',
  description: 'leadv2 Phase 1.5 Diverge as a workflow (judge-panel): N isolated frame-shifted generators + 1 critic that scores/clusters/flags traps/deepens top-K. Model-pinned.',
...
const JUDGE_SCHEMA = {
  ...
  properties: {
    ranked: { type: 'array', items: { type: 'object', additionalProperties: false,
      properties: { idea: { type: 'string' }, score: { type: 'number' }, cluster: { type: 'string' }, trap: { type: 'boolean' }, why: { type: 'string' } },
      required: ['idea', 'score'] } },
...
let judged = await agent(
  ...
  `Flag traps (plausible-but-doomed). Recommend ONE (or a synthesis) and say why. ` +
...
return {
  ...
  top: judged ? (judged.ranked || []).slice(0, 3) : [],
}
